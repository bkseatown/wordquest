...
const TRACKS = [
  {
    ...
    energy: 'low',
    prog: [[0, 3, 7], [5, 8, 12], [7, 10, 14], [3, 7, 10]],
  ...
  },
  {
    ...
    energy: 'low',
    prog: [[0, 4, 7], [2, 5, 9], [5, 9, 12], [4, 7, 11]],
  ...
  },
  {
    ...
    energy: 'mid',
    prog: [[0, 3, 7], [2, 5, 9], [7, 10, 14], [5, 8, 12]],
    melody: [0, 2, 3, 5, 7, 10, 7, 5]
  },
  {
    title: 'After Class Calm',
    slug: 'after-class-calm',
    bpm: 84,
    rootMidi: 48,
    modes: ['focus', 'chill', 'coffee', 'fantasy'],
    energy: 'low',
    prog: [[0, 3, 7], [7, 10, 14], [2, 5, 9], [5, 8, 12]],
    melody: [0, 3, 7, 8, 10, 8, 7, 3]
  },
  {
    title: 'Notebook Nights',
    slug: 'notebook-nights',
    bpm: 92,
    rootMidi: 52,
    modes: ['focus', 'chill', 'lofi', 'upbeat'],
    energy: 'mid',
    prog: [[0, 4, 7], [5, 9, 12], [7, 11, 14], [2, 5, 9]],
  ...
  },
  {
    ...
    modes: ['focus', 'chill', 'lofi', 'coffee'],
    energy: 'low',
    prog: [[0, 3, 7], [5, 8, 12], [4, 7, 11], [3, 7, 10]],
...
function addTone(buffer, startSec, durSec, freq, amp, wave = 'sine', detune = 0) {
  ...
  for (let i = 0; i < maxLen; i += 1) {
    ...
    const wobble = detune ? osc('sine', twoPi * (freq + detune) * t) * 0.2 : 0;
    buffer[start + i] += (base + wobble) * amp * env;
  }
}

function addKick(buffer, startSec, amp = 0.56) {
  const start = Math.floor(startSec * SAMPLE_RATE);
  ...
  for (let i = 0; i < maxLen; i += 1) {
    ...
    const freq = 140 - (95 * progress);
    const env = Math.exp(-7.4 * progress);
    const body = Math.sin(2 * Math.PI * freq * t);
    const click = Math.sin(2 * Math.PI * 180 * t) * Math.exp(-64 * progress);
    buffer[start + i] += (body * 0.94 + click * 0.16) * amp * env;
  }
}

function addSnare(buffer, startSec, rand, amp = 0.2) {
  const start = Math.floor(startSec * SAMPLE_RATE);
  ...
  for (let i = 0; i < maxLen; i += 1) {
    const progress = i / maxLen;
    const env = Math.exp(-10.8 * progress);
    const noise = (rand() * 2 - 1);
    const tone = Math.sin(2 * Math.PI * 170 * (i / SAMPLE_RATE)) * 0.22;
    buffer[start + i] += (noise * 0.9 + tone) * amp * env;
  }
}

function addHat(buffer, startSec, rand, amp = 0.09) {
  const start = Math.floor(startSec * SAMPLE_RATE);
  const len = Math.floor(SAMPLE_RATE * 0.075);
  const maxLen = Math.min(len, buffer.length - start);
  if (maxLen <= 0) return;
  let hp = 0;
  for (let i = 0; i < maxLen; i += 1) {
...
function addVinylNoise(buffer, rand, amp = 0.0042) {
  let lp = 0;
...
function renderTrack(track) {
  ...
  for (let bar = 0; bar < bars; bar += 1) {
    ...
    chord.forEach((interval, idx) => {
      ...
      addTone(samples, barStart, barSec * 0.96, midiToHz(midi), 0.045, 'triangle', idx === 0 ? 0.16 : 0.1);
      addTone(samples, barStart + 0.02, barSec * 0.88, midiToHz(midi + 12), 0.018, 'sine', 0);
    });

    for (let beat = 0; beat < 4; beat += 1) {
      const beatStart = barStart + beat * beatSec;
      const bassMidi = track.rootMidi + chord[0] - 12;
      addTone(samples, beatStart, beatSec * 0.58, midiToHz(bassMidi), 0.12, 'sine', 0.08);

      const onMainKick = beat === 0 || beat === 2;
      addKick(samples, beatStart, onMainKick ? 0.58 : 0.43);
      if (track.energy === 'mid' && beat === 3) addKick(samples, beatStart + (beatSec * 0.5), 0.3);

      if (beat === 1 || beat === 3) {
        addSnare(samples, beatStart + beatSec * 0.02, rand, track.energy === 'mid' ? 0.22 : 0.18);
      }

      addHat(samples, beatStart + beatSec * 0.5, rand, track.energy === 'mid' ? 0.11 : 0.085);
    }

    for (let step = 0; step < 8; step += 1) {
      const noteIndex = (bar * 8 + step) % melodyLen;
      const interval = track.melody[noteIndex];
      const swing = step % 2 === 1 ? beatSec * 0.03 : 0;
      const start = barStart + step * (beatSec / 2) + swing;
      const dur = (beatSec / 2) * (track.energy === 'mid' ? 0.8 : 0.74);
  ...
  addVinylNoise(samples, rand, track.energy === 'mid' ? 0.0046 : 0.0038);

  for (let i = 0; i < samples.length; i += 1) {
    const t = i / SAMPLE_RATE;
    samples[i] += Math.sin(2 * Math.PI * 0.27 * t) * 0.0045;
  }
...
function main() {
  ...
  console.log(`Generated ${made.length} chill study beat tracks:`);
  made.forEach((entry) => {
